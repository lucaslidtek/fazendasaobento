...
export default function CulturaDetalhes() {
  ...
  if (isError || !cultura) {
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center py-20">
          <h2 className="text-2xl font-bold text-slate-800 mb-4">Cultura não encontrada</h2>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-3xl font-bold text-slate-900 leading-tight">{cultura.name}</h1>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex flex-col p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors gap-2">
                       <p className="text-sm font-bold text-slate-800">Total Produzido na Fazenda</p>
                    ...
                    <div className="flex flex-col p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors gap-2">
                       <p className="text-sm font-bold text-slate-800">Total Destinado em Área</p>
        ...
        <TabsContent value="colheitas" className="space-y-4">
          <div className="flex items-center gap-2 mb-4 mt-2">
            ...
            <h2 className="text-lg font-bold text-slate-800">Histórico de Colheitas - {cultura.name}</h2>
          </div>
          ...
          {harvestStats.harvests.length > 0 ? (
            <>
                  ...
                  <TableBody>
                    {harvestStats.harvests.map((h: any, i: number) => (
                      <TableRow key={h.id || i} className="hover:bg-muted/30 cursor-pointer" onClick={() => window.location.href = `/colheita`}>
                        <TableCell className="font-bold text-slate-900">{h.area}</TableCell>
              ...
              <div className="md:hidden space-y-4">
                {harvestStats.harvests.map((h: any, i: number) => (
                  <Card key={h.id || i} className="bg-white border-slate-200 hover:border-primary/30 transition-all cursor-pointer touch-card" onClick={() => window.location.href = `/colheita`}>
                    <CardContent className="p-5 flex flex-col gap-3">
                      <div className="flex justify-between items-start">
                        <div>
                          <div className="text-base font-bold text-slate-900 leading-tight">{h.area}</div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-2 pt-3 border-t border-slate-100">
                        ...
                        <div>
                          <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Área (ha)</p>
                          <p className="font-bold text-slate-800">{h.areaHectares.toLocaleString()}</p>
                        </div>
                        <div>
            ...
            <div className="py-20 text-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
              ...
              <p className="text-slate-500 font-medium text-lg">Nenhuma colheita registrada para esta cultura.</p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="insumos" className="space-y-4">
          <div className="flex items-center gap-2 mb-4 mt-2">
            ...
            <h2 className="text-lg font-bold text-slate-800">Insumos Aplicados nesta Cultura</h2>
          </div>
          ...
          {usedProducts.length > 0 ? (
            ...
            <div className="py-20 text-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
              ...
              <p className="text-slate-500 font-medium text-lg">Nenhum insumo aplicado especificamente para questa cultura de forma documentada no estoque.</p>
            </div>
          )}
        </TabsContent>
