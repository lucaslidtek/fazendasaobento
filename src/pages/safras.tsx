...
export const apiFetchSafras = async () => {
  await fakeDelay();
  return [...MOCK_SAFRAS];
};

export const apiFetchSafraById = async (id: number) => {
  await fakeDelay();
  const safra = MOCK_SAFRAS.find((s) => s.id === id);
  if (!safra) throw new Error("Safra não encontrada");
  return safra;
};

const apiCreateSafra = async (data: any) => {
  await fakeDelay();
  const newSafra = { ...data, id: nextId++ };
  MOCK_SAFRAS.push(newSafra);
  return newSafra;
};
...
export const apiUpdateSafra = async ({ id, data }: { id: number; data: any }) => {
  ...
  MOCK_SAFRAS[index] = { ...MOCK_SAFRAS[index], ...data };
...
export default function Safras() {
  ...
  const { data: records, isLoading } = useQuery({
    queryKey: ["/safras"],
    queryFn: apiFetchSafras
  });

  const createMutation = useMutation({
    mutationFn: apiCreateSafra,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/safras"] });
      toast({ title: "Safra cadastrada." });
      closeForm();
      form.reset();
    },
  ...
  const handleDelete = (id: number) => {
    if (confirm("Tem certeza que deseja excluir esta safra?")) deleteMutation.mutate(id);
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Gestão dos períodos de safras da fazenda.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto no-print">
          ...
          <Dialog open={isDialogOpen} onOpenChange={(open) => { if (!open) closeForm(); else setIsDialogOpen(true); }}>
            ...
            <DialogContent className="sm:max-w-[400px]">
              <DialogHeader>
                <DialogTitle className="text-xl">{editingRecord ? "Editar Safra" : "Nova Safra"}</DialogTitle>
              </DialogHeader>
              <div className="mt-2">
      ...
      <div className="hidden md:block bg-card rounded-2xl border overflow-hidden">
        {isLoading ? (
          ...
          <Table>
            ...
            <TableBody>
              {records?.length === 0 && (
                <TableRow><TableCell colSpan={4} className="text-center py-10 text-muted-foreground">Nenhuma safra cadastrada.</TableCell></TableRow>
              )}
              {records?.map((r: any) => (
                <TableRow key={r.id} className="hover:bg-muted/30 cursor-pointer" onClick={(e) => {
                  // Prevenir navegação se clicou num botão de ação
                  if ((e.target as HTMLElement).closest("button")) return;
                  navigateToDetails(r.id);
                }}>
                  ...
                  <TableCell onClick={(e) => e.stopPropagation()} className="no-print">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); handleEditOpen(r, false); }} className="rounded-full w-8 h-8 text-muted-foreground hover:text-foreground">
                        <Pencil className="w-4 h-4" />
                      </Button>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="icon" onClick={(e) => Object.assign(e, { isDropdownTrigger: true })} className="rounded-full w-8 h-8 text-muted-foreground hover:text-foreground">
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex flex-col items-center gap-2">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={(e) => { e.stopPropagation(); navigateToDetails(r.id); }} className="gap-2 cursor-pointer">
                      <ChevronRight className="w-4 h-4" />
                      Detalhes
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleEditOpen(r, true); }} className="gap-2 cursor-pointer">
                      <Pencil className="w-4 h-4" />
                      Editar
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleDelete(r.id); }} className="gap-2 cursor-pointer text-destructive focus:text-destructive">
                      <Trash2 className="w-4 h-4" />
                      Excluir
                    </DropdownMenuItem>
                  </DropdownMenuContent>
        ...
        <Sheet open={isSheetOpen} onOpenChange={(open) => { if (!open) closeForm(); else setIsSheetOpen(true); }}>
          ...
          <SheetContent side="bottom" className="rounded-t-3xl px-4 pb-8 max-h-[92vh] overflow-y-auto">
            ...
            <SheetHeader className="text-left mb-4">
              <SheetTitle className="text-lg">{editingRecord ? "Editar Safra" : "Nova Safra"}</SheetTitle>
            </SheetHeader>
            <FormContent {...formProps} />
          </SheetContent>
        </Sheet>
